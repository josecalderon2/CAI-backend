...
export class SistemaEvaluacionService {
  ...
  /**
   * Actualiza una nota mensual por ID usando formato simplificado
   * Solo actualiza actividades y exámenes, recalcula automáticamente todos los promedios
   */
  async actualizarNotaSimplificadaPorId(
    ...
    // Construir DTO completo para recalcular
    const dtoCompleto = {
      id_alumno: notaExistente.id_alumno,
      id_asignatura: notaExistente.id_asignatura,
      mes: notaExistente.mes,
      trimestre: notaExistente.trimestre,
      anio_academico: notaExistente.anio_academico,
      actividades: dto.actividades.map((act: any) => ({
        id_tipo_actividad: parseInt(act.id_tipo_actividad),
        numero_actividad: act.numero_actividad
          ? parseInt(act.numero_actividad)
    ...
    // Reutilizar el método calcularNotaMensual que hace upsert
    const resultado = await this.calcularNotaMensual(dtoCompleto);

    // Convertir respuesta a formato simplificado
    return {
      ...resultado,
      mes_numerico: this.convertirNombreMesANumerico(notaExistente.mes),
      anio: parseInt(notaExistente.anio_academico),
    };
  }
}
