...
export class SistemaEvaluacionController {
  ...
  @Get('formato-evaluacion/asignatura/:id_asignatura')
  @ApiOperation({
    summary: '🎨 Obtener FORMATO/ESTRUCTURA de evaluación por asignatura',
    description:
      '**Este es el endpoint que el frontend debe usar para renderizar el formulario de notas.**\n\n' +
      'Devuelve la estructura EXACTA de actividades que se deben ingresar:\n\n' +
      '**Para BÁSICA:**\n' +
      '- Formato FIJO: Tarea 1, Revisión de libros y cuadernos, Tarea 2, Laboratorio escrito\n' +
      '- Orden predefinido\n' +
      '- El frontend renderiza los campos en ese orden exacto\n\n' +
      '**Para BACHILLERATO:**\n' +
      '- Formato CATEGORIZADO: Debe tener al menos una actividad de cada categoría\n' +
      '- El frontend permite agregar múltiples actividades por categoría\n\n' +
      '**Ventajas:**\n' +
      '- ✅ Frontend sabe exactamente qué mostrar\n' +
      '- ✅ Incluye validaciones (campos requeridos)\n' +
      '- ✅ Incluye fórmula de cálculo\n' +
      '- ✅ Incluye información de exámenes requeridos',
  })
  @ApiParam({
    name: 'id_asignatura',
    description: 'ID de la asignatura',
  ...
  @ApiResponse({
    status: 200,
    description: 'Formato/estructura de evaluación',
    schema: {
      type: 'object',
      example: {
        nivel: 'BASICA',
  ...
  async obtenerFormatoEvaluacionPorAsignatura(
    ...
    return this.sistemaEvaluacionService.obtenerFormatoEvaluacionPorAsignatura(
      id_asignatura,
    );
  }
